...
public class Events {
	...
	public class InputDevice {
		...
		final int EV_KEY = 0x01, EV_REL = 0x02, EV_ABS = 0x03, REL_X = 0x00, REL_Y = 0x01, REL_Z = 0x02, EV_SYN = 0x00,
				BTN_TOUCH = 0x14a;// 330
		 ...
		 * 灞忓箷鐐瑰嚮浜嬩欢
		 */
		public int SendTouchAbs(int x, int y) {
			Log.e("SendTouchAbsCoord", "testtest-SendTouchAbsCoord");
			intSendEvent(m_nId, EV_ABS, 57, 157);
			intSendEvent(m_nId, EV_KEY, 330, 1);
			intSendEvent(m_nId, EV_KEY, 325, 1);
			intSendEvent(m_nId, EV_ABS, 53, x);
			intSendEvent(m_nId, EV_ABS, 54, y);
			intSendEvent(m_nId, EV_SYN, 0, 0);
			intSendEvent(m_nId, EV_KEY, 330, 0);
			intSendEvent(m_nId, EV_KEY, 325, 0);
			intSendEvent(m_nId, EV_SYN, 0, 0);
		 ...
		 * 灞忓箷婊戝姩浜嬩欢
		 */
		public void SendSwipeAbs(final int startx,final int starty, final int endx, final int endy,final Long time) {

			LogUtils.loge("SendSwipeAbs", "testtest-SendSwipeAbs1111");
			
			 new Thread(new Runnable() {
	                @Override
	                public void run() {
	                    LogUtils.loge("SendSwipeAbs", "test start SendSwipeAbs");
	                    
	                    intSendEvent(m_nId, EV_KEY, 330, 1); 
	                    intSendEvent(m_nId, EV_KEY, 325, 1);
	                    intSendEvent(m_nId, EV_ABS, 53 , startx);
	                    intSendEvent(m_nId, EV_ABS, 54 , starty);
	                    intSendEvent(m_nId, EV_SYN, 0 , 0);
	                    Looper.prepare();
	                    new Handler().postDelayed(new Runnable() {
	                        public void run() {
	                            intSendEvent(m_nId, EV_ABS, 53, endx);
	                            intSendEvent(m_nId, EV_ABS, 54, endy);
	                            intSendEvent(m_nId, EV_SYN, 0, 0);
	                            intSendEvent(m_nId, EV_KEY, 330, 0);
	                            intSendEvent(m_nId, EV_KEY, 325, 0);
	                            intSendEvent(m_nId, EV_SYN, 0, 0);
	                            LogUtils.loge("SendSwipeAbs", "test end SendSwipe time" + time);
	                        }
	                    }, time);
			...
			Timer timer=new Timer();//瀹炰緥鍖朤imer绫�   
		 ...
		 * 灞忓箷闀挎寜浜嬩欢
		 */
		public void SendLongPressAbs(int x, int y, int time) {
			intSendEvent(m_nId, EV_ABS, 57, 193);
			intSendEvent(m_nId, EV_KEY, 330, 1);
			intSendEvent(m_nId, EV_KEY, 325, 1);
			intSendEvent(m_nId, EV_ABS, 53, x);
			intSendEvent(m_nId, EV_ABS, 54, y);
		 ...
		 * 瀹炰綋閿洏 --home閿�
		 */
		public void SendTouchHome() {
			Log.e("SendAbs", "testtest-SendTouchHome");
			intSendEvent(m_nId, EV_KEY, 96, 1);
			intSendEvent(m_nId, EV_SYN, 0, 0);
			intSendEvent(m_nId, EV_KEY, 96, 0);
			intSendEvent(m_nId, EV_SYN, 0, 0);
		}
		 ...
		 * 瀹炰綋閿洏 --back閿�
		 */
		public void SendTouchBack() {
			Log.e("SendAbs", "testtest-SendTouchBack");
			intSendEvent(m_nId, EV_KEY, 158, 1);
			intSendEvent(m_nId, EV_SYN, 0, 0);
			intSendEvent(m_nId, EV_KEY, 158, 0);
			intSendEvent(m_nId, EV_SYN, 0, 0);
		}
		 ...
		 * 瀹炰綋閿洏 --menu閿�
		 */
		public void SendTouchMenu() {
			Log.e("SendAbs", "testtest-SendTouchMenu");
			intSendEvent(m_nId, EV_KEY, 139, 1);
			intSendEvent(m_nId, EV_SYN, 0, 0);
			intSendEvent(m_nId, EV_KEY, 139, 0);
			intSendEvent(m_nId, EV_SYN, 0, 0);
		}
		 ...
		 * 闊抽噺閿�
		 */
		public void SendTouchVoice() {
			Log.e("SendAbs", "testtest-SendTouchMenu");
			// 闊抽噺鎸夐挳
			intSendEvent(m_nId, EV_KEY, 115, 1);
			intSendEvent(m_nId, EV_SYN, 0, 0);
			intSendEvent(m_nId, EV_KEY, 115, 0);
			intSendEvent(m_nId, EV_SYN, 0, 0);
		}
		 ...
		 * 
		 * @param forceOpen
		 *            will try to set permissions and then reopen if first open attempt
		 *            fails
		 * @return true if input event node has been opened
		 */
		public boolean Open(boolean forceOpen) {
			int res = OpenDev(m_nId);
			// if opening fails, we might not have the correct permissions, try changing 660
			// to 666
			if (res != 0) {
